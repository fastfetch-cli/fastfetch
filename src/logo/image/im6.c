...
static FFLogoImageResult im6EncodeImage(FFLogoRequestData* requestData, const char* magick, uint32_t magickLength, void** outBlob, size_t* outLength) {
    // clang-format off
...
bool ffImageSixelEncodeIM6(FFLogoRequestData* requestData, FFstrbuf* out, const char** error) {
    FF_AUTO_FREE void* blob = nullptr;
    size_t length = 0;
    FFLogoImageResult result = im6EncodeImage(requestData, "SIXEL", 6, &blob, &length);
    if (result != FF_LOGO_IMAGE_RESULT_SUCCESS) {
        setError(result, error);
        return false;
    }
