...
void ffLogoPrintChars(FFinstance* instance, const char* data, bool doColorReplacement)
{
    ...
    while (*data != '\0')
    {
        ...
        if (*data == '\033' && *(data + 1) == '[')
        {
            ...
            // We have a valid control sequence, print it and continue with the next char
            if (isascii(*data))
            {
            ...
            // Invalid control sequence, try to get the most accurate length
        ...
        // We have a fastfetch color placeholder. Replace it with the escape sequence, don't increase the line length
        if (doColorReplacement && *data == '$')
        {
            ++data;

            // If we have $$, or $\0, print it as a single $
            if (*data == '$' || *data == '\0')
            {
                putchar('$');
                ++currentlineLength;
        ...
        if (codepoint <= 127)
            ...
            bytes = 1; // Invalid UTF-8, print it as is, byte by byte
...
static bool logoPrintFileIfExists(FFinstance* instance, bool doColorReplacement, bool raw)
{
    ...
    if (!ffAppendFileBuffer(instance->config.logo.source.chars, &content))
    {
        ...
        fputs("Logo: Failed to load file content from the logo source\n", stderr);
        return false;
    }
...
void ffLogoPrint(FFinstance* instance)
{
    ...
    // We also don't need to set the main color because it won't be printed anyway.
    ...
    // If the source and source type are set to something other than auto, always print with the set type.
    ...
    // If the source matches the name of a builtin logo, print it and return.
    ...
    // Terminal emulators that support the kitty graphics protocol.
    bool supportsKitty =
