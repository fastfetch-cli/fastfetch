...
// Solaris 11.4 compatible implementation
const char* zpool_get_state_str(zpool_handle_t* zpool)
{
    ...
    nvlist_t *nvroot;
    if (nvlist_lookup_nvlist(zpool_get_config(zpool, NULL),
    ...
    if (nvlist_lookup_uint64_array(nvroot, ZPOOL_CONFIG_VDEV_STATS,
                                   (uint64_t**)&vs, &vsc) != 0)
#else
    if (nvlist_lookup_uint32_array(nvroot, ZPOOL_CONFIG_VDEV_STATS,
                                   (uint32_t**)&vs, &vsc) != 0)
#endif
        return "UNKNOWN";
...
#else // non-Solaris (Linux/OpenZFS)
#include <libzfs.h>
...
const char* zpool_get_state_str(zpool_handle_t* zpool)
{
    ...
    else
    {
        ...
        {
            nvlist_t *nvroot = nvlist_lookup_nvlist(zpool_get_config(zpool, NULL), ZPOOL_CONFIG_VDEV_TREE);
            uint_t vsc;
            vdev_stat_t *vs;
#ifdef __x86_64__
...
static int enumZpoolCallback(zpool_handle_t* zpool, void* param)
{
    ...
    uint64_t fragmentation = data->ffzpool_get_prop_int(zpool, ZPOOL_PROP_DELEGATION, &source);
