...
static FFCodecType ffCodecProfileToType(const GUID& profile) {
    // clang-format off
...
const char* ffDetectCodec(FF_A_UNUSED FFCodecOptions* options, FFlist* result /*list of FFCodecResult*/) {
    ...
    for (UINT adapterIndex = 0;; ++adapterIndex) {
        ...
        for (UINT profileIndex = 0; profileIndex < profileCount; ++profileIndex) {
            ...
            gpuResult->decoders = (FFCodecType) (((uint32_t) gpuResult->decoders) | ((uint32_t) ffCodecProfileToType(profile)));
        }
    }
